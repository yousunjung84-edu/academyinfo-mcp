...
Release conclusion: **HOLD — `BLOCKED_PENDING_BACKEND_SELECTION`; first-publication bootstrap `HOLD/UNSUPPORTED`**.
...
When public npm `latest` is absent, the current candidate/client/promotion receipt and workflow chain cannot execute a first-ever publication. Each stage requires exact SemVer `expected_previous_latest`; promotion also parses an existing public `dist-tags.latest` and validates it as SemVer. A separately reviewed canonical absent-`latest` contract is required before an initial `0.1.0` path is executable. No sentinel, placeholder, candidate version, or fabricated SemVer may be used as predecessor evidence. This migration still provides useful read-only verification, anonymous registry observation, sanitized evidence, and removal of Action registry authority; it does not bootstrap first publication. Once canonical support lands, human terminal ceremonies remain the only permitted registry writes.
...
| First-publication bootstrap | HOLD/UNSUPPORTED | A separately reviewed canonical absent-`latest` receipt, verifier, and workflow contract; the current exact-SemVer predecessor chain cannot represent an absent public `latest` |
...
Under the current predecessor contract, a candidate workflow may run only when anonymous public history supplies a real previous `latest` SemVer. If `latest` is absent, first-publication bootstrap remains `HOLD/UNSUPPORTED`; neither this migration nor the manual ceremony can bypass the missing canonical contract.
...
Release remains on hold at `BLOCKED_PENDING_BACKEND_SELECTION`, and first-publication bootstrap remains `HOLD/UNSUPPORTED` until canonical absent-`latest` support is separately reviewed and implemented. The last-known-good public package and data, where they exist, must remain available until all applicable evidence gates, the pre-existing bound promotion authorization, successful readiness verification, and human registry ceremonies succeed.
