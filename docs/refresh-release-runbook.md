...
Current first-publication bootstrap is **HOLD/UNSUPPORTED**. If public npm `latest` is absent, the current chain cannot execute a first-ever publication: candidate, client, and promotion contracts require exact SemVer `expected_previous_latest`, while promotion parses an existing public `dist-tags.latest` and validates it as SemVer. Stop before candidate dispatch until a separately reviewed canonical absent-`latest` receipt, verifier, and workflow contract lands. Never supply a sentinel, placeholder, candidate version, or fabricated SemVer as predecessor evidence. The read-only migration remains useful, but it does not make initial `0.1.0` publication executable; after that prerequisite is implemented, the human ceremonies remain the only permitted registry writes.
