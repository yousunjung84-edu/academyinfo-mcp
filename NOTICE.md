...
university disclosure indicators.
...
v0.1 bundles only a normalized derivative seed database for dataset `15118998`.
...
The seed package must not include raw XLSX or CSV source files.

## Non-Bundled Data Notice

Dataset `15139279` is employment data and is non-bundled v0.3 backlog only.
It must not appear in npm package artifact paths as data.
